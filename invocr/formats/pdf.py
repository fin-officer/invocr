...
class PDFProcessor:
        ...
        for pattern in table_patterns:
            ...
            for match in table_matches:
                ...
                for i, header in enumerate(headers):
                    ...
                    if any(p in header_lower for p in ['desc', 'item', 'product', 'service']):
                ...
                for line in lines[1:]:
                    ...
                    if re.search(r'(?i)total|subtotal|balance|amount due|grand total', line):
                        continue
                    ...
                    for i, col in enumerate(columns):
                        if i in field_mapping and col.strip():
                            ...
                            if field == 'quantity':
                                try:
                                    item[field] = float(re.sub(r'[^\d.]', '', value) or '1')
                                    valid_item = True
                    ...
                    if valid_item and 'description' in item:
                        ...
                        if 'quantity' in item and 'unit_price' in item and 'total' not in item:
                            try:
                                item['total'] = item['quantity'] * item['unit_price']
                            except (TypeError, KeyError):
                                pass
                        elif 'quantity' in item and 'total' in item and 'unit_price' not in item and item['quantity'] != 0:
                            try:
                ...
                for match in matches:
                    try:
                        if len(match.groups()) >= 3:
                            ...
                            item.unit_price = float(match.group(3).replace(',', '').replace('$', '').replace('€', '').replace('£', '').replace('¥', ''))
                            
                            # If there's a 4th group, it's the total, otherwise calculate it
                            if len(match.groups()) >= 4:
                                item.total = float(match.group(4).replace(',', '').replace('$', '').replace('€', '').replace('£', '').replace('¥', ''))
                            else:
                                item.total = item.quantity * item.unit_price
    ...
    def _extract_totals(self, text: str) -> Dict[str, Any]:
        ...
        for pattern in self.patterns['amount']:
            ...
            if matches:
                ...
                try:
                    totals.total = float(amount_str.replace('$', '').replace('€', '').replace('£', '').replace('¥', ''))
                    break
                except (ValueError, AttributeError):
                    continue
    ...
    def to_json(self, pdf_path: Union[str, Path], output_path: Union[str, Path, None] = None) -> str:
        """
